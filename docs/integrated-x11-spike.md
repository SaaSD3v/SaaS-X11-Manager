...
`139f2197e6093d04d5df1400baa998bf1fb07b3c`. The upstream project is GPL-3.0
licensed; this spike keeps that source and its license in the submodule so
licensing can be reviewed explicitly before any future product decision.

The upstream `lorie/build.gradle` currently targets a newer Gradle API than the
Manager. Rather than upgrading the whole application or modifying the pinned
submodule, this branch builds the same Lorie Java/resources/CMake sources through
a local `:embedded-lorie` Android-library wrapper compatible with the Manager's
existing Gradle/AGP toolchain. The wrapper also fixes Lorie's host application ID
to `com.saas.x11manager`, which is what its `CmdEntryPoint` broadcasts back to.

CI checks out the upstream pin recursively so the native Xorg/Lorie submodules
...
sessions and by the Display tab; stopping the server leaves the cache intact.

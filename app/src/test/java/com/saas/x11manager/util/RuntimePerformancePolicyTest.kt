...
class RuntimePerformancePolicyTest {
    ...
    fun fixedX0UsesOneBoundedProbeAndNoWholeProcProcessScan() {
        ...
        assertTrue(manager.contains("private fun waitForServerRuntime()"))
        assertTrue(manager.contains("val runtime = waitForServerRuntime()"))
        assertTrue(manager.contains("cat /proc/net/unix 2>/dev/null"))
        assertTrue(manager.contains("var backoffMs = 250L"))
        assertTrue(manager.contains("coerceAtMost(3_000L)"))
    ...
    fun stopAllAndComposeCollectorsAvoidRedundantBackgroundWork() {
        ...
        assertTrue(manager.contains("survivingOwners"))
        assertTrue(manager.contains("X0 retained because running owner(s) remain"))
        assertTrue(manager.contains("clearManualX11Config(container.name, logger)"))

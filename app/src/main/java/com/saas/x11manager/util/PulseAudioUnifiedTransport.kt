...
/**
 * Unified HOST/NAT transport for the single Manager-owned PulseAudio core.
 *
 * Physical baseline proven on-device:
 * - one AAudio_sink PulseAudio core
 * - one private UNIX control socket and one private 256-byte cookie
 * - HOST listener: 127.0.0.1:4713
 * - NAT listener: 172.28.0.1:4713
 * - both TCP listeners can coexist on the same daemon
 *
 * The core is prepared by PulseAudioFixManager before graphical startup. This
 * object never starts, stops or replaces that daemon. It uses Termux's official
 * RunCommandService only to run pactl in the real Termux app context, matching
 * the physical manual test. Container/X11/VNC lifecycle is never touched here.
 */
object PulseAudioUnifiedTransport {
    ...
    private const val COMMANDS = "$STATE/run-command-control"
    ...
    private data class ListenerReady(val port: Int, val sink: String)
    ...
    suspend fun finalizeAfterContainerReady(
        ...
        val sink = verifyCoreThroughTermux(owner, logger)
        ...
        } ?: return@withContext fail(logger, "Automatic HOST/NAT audio endpoint discovery failed")

        logger?.i("[CTX] Audio net_mode: $mode")
        logger?.i("[CTX] Audio host endpoint: $endpoint (port selected automatically)")

        val listener = selectAndEnsureListener(owner, endpoint, sink, logger)
            ?: return@withContext fail(logger, "Could not create a usable authenticated audio listener")
        val server = "tcp:$endpoint:${listener.port}"

        if (!verifyContainerClient(containerName, server)) {
            val octal = cookieOctalThroughTermux(owner, logger)
                ?: return@withContext fail(logger, "Could not prepare the private PulseAudio cookie for the container")
            if (!installContainerClient(containerName, server, octal, logger)) {
                return@withContext fail(logger, "Container audio client configuration failed")
            }
        } else {
    ...
    private suspend fun verifyCoreThroughTermux(owner: TermuxOwner, logger: ContainerLogger?): String? {
        val body = """
            CONTROL=${q(CONTROL)}; COOKIE=${q(COOKIE)}
            info=${'$'}(PULSE_SERVER="unix:${'$'}CONTROL" PULSE_COOKIE="${'$'}COOKIE" pactl info 2>&1) || {
                printf '%s\n' "${'$'}info" >&2; echo 'ERR|CORE|connect'; exit 1;
            }
            sinks=${'$'}(PULSE_SERVER="unix:${'$'}CONTROL" PULSE_COOKIE="${'$'}COOKIE" pactl list short sinks 2>&1) || {
            ...
            echo "OK|CORE|${'$'}sink"
        """.trimIndent()
        val result = runTermuxCommand(owner, "core-probe", body, 4000, logger) ?: return null
        if (!result.startsWith("OK|CORE|")) return null
        return result.substringAfter("OK|CORE|").takeIf { it in setOf("AAudio_sink", "OpenSL_ES_sink") }
    }
    ...
    private suspend fun selectAndEnsureListener(
        ...
        for (port in BASE_PORT..max) {
            ...
            val ready = ensureListenerThroughTermux(owner, ip, port, expectedSink, logger)
            if (ready != null) {
    ...
    private suspend fun ensureListenerThroughTermux(
        owner: TermuxOwner,
        ip: String,
        port: Int,
        expectedSink: String,
        ...
        val body = """
            ...
            unix="unix:${'$'}CONTROL"; tcp="tcp:${'$'}IP:${'$'}PORT"

            info=${'$'}(PULSE_SERVER="${'$'}tcp" PULSE_COOKIE="${'$'}COOKIE" pactl info 2>/dev/null || true)
            if printf '%s\n' "${'$'}info" | grep -Fq "Default Sink: ${'$'}EXPECTED"; then
                echo "OK|REUSE|${'$'}EXPECTED"
                exit 0
            fi
            ...
            rc=${'$'}?
            if [ "${'$'}rc" -ne 0 ]; then
                cat "${'$'}err" >&2 2>/dev/null || true; rm -f "${'$'}err" 2>/dev/null || true
                echo "ERR|LOAD|${'$'}rc"; exit 4
            fi
            rm -f "${'$'}err" 2>/dev/null || true
            case "${'$'}id" in ''|*[!0-9]*) echo "ERR|LOAD|id:${'$'}id"; exit 5 ;; esac

            j=0
            while [ "${'$'}j" -lt 20 ]; do
                info=${'$'}(PULSE_SERVER="${'$'}tcp" PULSE_COOKIE="${'$'}COOKIE" pactl info 2>/dev/null || true)
                if printf '%s\n' "${'$'}info" | grep -Fq "Default Sink: ${'$'}EXPECTED"; then
                    echo "OK|READY|${'$'}id|${'$'}EXPECTED"
                    exit 0
                fi
                sleep .1; j=${'$'}((j+1))
            done
            ...
            echo "ERR|VERIFY|${'$'}id"; exit 6
        """.trimIndent()

        val result = runTermuxCommand(owner, "listener-${ip.replace('.', '_')}-$port", body, 5000, logger)
            ?: return null
        return when {
            result.startsWith("OK|REUSE|") -> {
                val sink = result.substringAfter("OK|REUSE|")
                if (sink != expectedSink) null else {
                    logger?.i("[+] Reusing authenticated PulseAudio listener on $ip:$port")
                    ListenerReady(port, sink)
                }
            }
            result.startsWith("OK|READY|") -> {
                val p = result.split('|')
                val sink = p.getOrNull(3).orEmpty()
                if (p.getOrNull(2)?.toIntOrNull() == null || sink != expectedSink) null else {
    ...
    private suspend fun cookieOctalThroughTermux(owner: TermuxOwner, logger: ContainerLogger?): String? {
        val body = """
            COOKIE=${q(COOKIE)}
            ...
            printf 'OK|COOKIE|%s\n' "${'$'}octal"
        """.trimIndent()
        val result = runTermuxCommand(owner, "cookie", body, 3000, logger) ?: return null
        return result.takeIf { it.startsWith("OK|COOKIE|") }?.substringAfter("OK|COOKIE|")?.takeIf { it.isNotBlank() }
    }
    ...
    private suspend fun ensureRunPermission(logger: ContainerLogger?): Boolean {
        ...
        val r = try { Shell.cmd("pm grant ${q(context.packageName)} ${q(RUN_PERMISSION)} 2>&1").exec() } catch (_: Exception) { null }
        if (context.checkSelfPermission(RUN_PERMISSION) == PackageManager.PERMISSION_GRANTED) return true
    ...
    private fun prepareManagedStateOwnership(owner: TermuxOwner) {
        val command = """
            mkdir -p ${q(STATE)} ${q(COMMANDS)} || exit 1
            chown -R ${owner.uid}:${owner.gid} ${q(STATE)} 2>/dev/null || true
            chmod 700 ${q(STATE)} ${q(COMMANDS)} 2>/dev/null || true

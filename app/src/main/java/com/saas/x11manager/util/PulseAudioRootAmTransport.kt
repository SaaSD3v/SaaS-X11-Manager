...
/**
 * Final HOST/NAT transport for the single Manager-owned PulseAudio core.
 *
 * The core itself is prepared by [PulseAudioFixManager] before graphical startup.
 * This transport only controls that existing core through the real Termux app
 * context. RUN_COMMAND is dispatched with root `am startservice` so Android
 * background-service restrictions on the Manager process do not block it.
 *
 * The `am` exit status is intentionally advisory only. With the shell form of
 * RUN_COMMAND the authoritative result is the Termux-owned result file written
 * by the launched script. This class therefore always waits for that handshake.
 *
 * This class never starts/stops/restarts PulseAudio, DroidSpaces, X11 or VNC.
 */
object PulseAudioRootAmTransport {
    ...
    private const val COMMANDS = "$STATE/run-command-root-am"
    ...
    suspend fun finalizeAfterContainerReady(
        ...
        // Keep the manifest/runtime permission healthy even though root `am`
        // is the actual sender. This also preserves compatibility with Termux
        // builds that perform stricter caller checks.
        ...
        logger?.i("[CTX] RUN_COMMAND result authority: Termux file handshake")
    ...
    private suspend fun verifyCore(owner: TermuxOwner, logger: ContainerLogger?): String? {
        val body = """
            ...
            printf 'OK|CORE|%s\n' "${'$'}sink"
        """.trimIndent()
        val result = runTermux(owner, "core-probe", body, 5000, logger) ?: return null
    ...
    private suspend fun loadOrReuseListener(
        ...
        val result = runTermux(owner, "listener-${ip.replace('.', '_')}-$port", body, 5000, logger)
            ?: return null
    ...
    private suspend fun unloadListener(owner: TermuxOwner, moduleId: Int, logger: ContainerLogger?) {
        val body = """
            ...
            printf 'OK|UNLOAD|%s\n' "${'$'}ID"
        """.trimIndent()
        runTermux(owner, "unload-$moduleId", body, 2500, logger)
    }
    ...
    private suspend fun cookieOctal(owner: TermuxOwner, logger: ContainerLogger?): String? {
        val body = """
            ...
            printf 'OK|COOKIE|%s\n' "${'$'}octal"
        """.trimIndent()
        val result = runTermux(owner, "cookie", body, 3500, logger) ?: return null
    ...
    private fun prepareStateOwnership(owner: TermuxOwner) {
        val command = "mkdir -p ${q(STATE)} ${q(COMMANDS)}; " +
    ...
    /**
     * Dispatch through root `am startservice` and wait for the Termux-owned file
     * handshake. The `am` process exit code is not authoritative and is never
     * used as an early failure signal.
     */
    private suspend fun runTermux(
        ...
        """.trimIndent()
        val dispatch = try { Shell.cmd(start).exec() } catch (_: Exception) { null }

        // Do not return early on dispatch.isSuccess == false. On this device the
        // ActivityManager prints "Starting service: Intent ..." while libsu can
        // still report a non-success shell status. RUN_COMMAND through `am` has
        // no direct result channel; the result file below is authoritative.
        ...
        repeat(loops) {
            ...
            if (r?.isSuccess == true && r.out.isNotEmpty()) {
                ...
                try {
                    Shell.cmd("rm -f ${q(launcher)} ${q(resultFile)} 2>/dev/null || true").exec()
                } catch (_: Exception) { }
                return result
            }
            delay(100)
        ...
        dispatch?.out?.filter { it.isNotBlank() }?.takeLast(6)?.forEach { logger?.w("[RUN_COMMAND] $it") }
        dispatch?.err?.filter { it.isNotBlank() }?.takeLast(6)?.forEach { logger?.w("[RUN_COMMAND] $it") }
        try {
            Shell.cmd("rm -f ${q(launcher)} ${q(resultFile)} 2>/dev/null || true").exec()
        } catch (_: Exception) { }
        return null

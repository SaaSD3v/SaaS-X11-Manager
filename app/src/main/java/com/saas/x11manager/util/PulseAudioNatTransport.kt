...
object PulseAudioNatTransport {
    ...
    private data class Ready(val pid: Int, val sink: String, val port: Int, val groups: String) {
        val server: String get() = "tcp:172.28.0.1:$port"
    }
    ...
    private fun ensureRunPermission(logger: ContainerLogger?): Boolean {
        val context = X11Application.instance
    ...
    private fun ensureExternalAppsPolicy(owner: TermuxOwner, logger: ContainerLogger?): Boolean {
    ...
    private fun stopOwnedCoreRoot(logger: ContainerLogger?) {
        val command = """
    ...
    private suspend fun runTermuxCommand(
        ...
        // No root-written script file is created here. The only pre-dispatch
        // operation is ensuring our Manager-owned state belongs to Termux.
        prepareManagedStateOwnership(owner)
        ...
        try {
            // This matches Termux's documented Java example. RunCommandService
            // itself promotes its internal execution service as required.
    ...
    private fun logRunCommandDiagnostics(resultFile: String, logger: ContainerLogger?) {
        val command = """

...
object GraphicSessionUserManager {
    ...
    suspend fun prepareFreeForStart(
        ...
        val committed = if (requested.createIfMissing) {
            requested.copy(createIfMissing = false)
        } else {
            requested
        }
        if (!writePersistedSelection(info, committed)) {
            logger?.e("[-] Could not persist Free user selection for $containerName")
            return@withContext null
        }
        consumePreparedSelection(containerName, requested)

        logger?.i("[CTX] Graphic user: ${committed.userName}")
        logger?.i("[USER] ✓ Free display user ready")
        GraphicSessionUserPreparation(committed, persisted != committed)
    }

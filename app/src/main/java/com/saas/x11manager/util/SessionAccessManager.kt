        ...
        return when (accessMode) {
            SessionAccessMode.INTEGRATED_X11, SessionAccessMode.BOTH -> {
                ...
                DisplayLogDetails.x11(
                    logger = logger,
                    containerName = containerName,
                    monitorNumber = slot.monitorNumber,
                    displayName = slot.displayName,
                    pid = serverPid,
                    processName = slot.processName,
                    runtimeDir = slot.runtimeDir,
                    hostSocket = slot.socketFile,
                    containerSocket = "/tmp/.X11-unix/X${slot.number}",
                    state = "ready"
                )
            ...
            SessionAccessMode.VNC -> {
                logger?.i(LogLayout.SPACER)
                logger?.i("[VNC] Preparing standalone Free VNC transport")
                logger?.i("[VNC] • Server port: $vncPort")
                logger?.i("[VNC] • PC local ADB port: $vncAdbLocalPort")
                ...
                val reservedSlot = reservation.getOrNull()
                var vncStarted = false
                try {
                    if (!ensureIntegratedMonitorStoppedForVnc(containerName, reservedSlot, logger)) {
                        return false
                    }

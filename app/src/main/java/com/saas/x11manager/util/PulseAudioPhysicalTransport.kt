...
/**
 * HOST/NAT transport for the single Manager-owned PulseAudio core.
 *
 * Important: TCP listener creation is controlled through the real Termux app
 * context, but listener reachability is verified by the running DroidSpaces
 * container (the actual client). We deliberately do not require Termux itself
 * to connect back to the Android NAT gateway, because that loopback path is not
 * the data path used by the container and failed on the physical test device.
 *
 * This object never starts/stops/restarts PulseAudio, DroidSpaces, X11 or VNC.
 */
object PulseAudioPhysicalTransport {
    ...
    private const val COMMANDS = "$STATE/run-command-physical"
    ...
    private data class ListenerCandidate(
        val port: Int,
        val sink: String,
        val moduleId: Int?,
    ...
    suspend fun finalizeAfterContainerReady(
        ...
        for (port in BASE_PORT..max) {
            ...
            val candidate = createOrReuseListener(owner, endpoint, port, sink, logger)
            if (candidate == null) {
                if (port == BASE_PORT) {
            ...
            val clientReady = if (verifyContainerClient(containerName, server)) {
                logger?.i("[+] Persistent container audio client already configured")
                true
            } else {
                installContainerClient(containerName, server, octal, logger)
            }

            if (clientReady && verifyContainerClientDetailed(containerName, server, sink, logger)) {
                ...
                logger?.i("[+] Audio ready (${candidate.sink}, $server)")
                return@withContext true
            }

            logger?.w("[!] Container could not reach $server; trying another audio port")
            if (candidate.createdNow && candidate.moduleId != null) {
                unloadListenerThroughTermux(owner, candidate.moduleId, logger)
            }
        }
    ...
    private suspend fun createOrReuseListener(
        owner: TermuxOwner,
        ip: String,
        port: Int,
        expectedSink: String,
        ...
        val body = """
            ...
            ')
            case "${'$'}existing" in
                [0-9]*) echo "OK|REUSE|${'$'}existing|${'$'}EXPECTED"; exit 0 ;;
            esac
            ...
            modules=${'$'}(PULSE_SERVER="${'$'}unix" PULSE_COOKIE="${'$'}COOKIE" pactl list short modules 2>&1) || {
                printf '%s\n' "${'$'}modules" >&2; echo "ERR|VERIFY|${'$'}id|list"; exit 6;
            }
            ...
            ' || {
                ...
                echo "ERR|VERIFY|${'$'}id|missing"; exit 7;
            }

            echo "OK|CREATED|${'$'}id|${'$'}EXPECTED"
        """.trimIndent()

        val result = runTermuxCommand(owner, "listener-${ip.replace('.', '_')}-$port", body, 3500, logger)
            ?: return null
        ...
        return when (parts.getOrNull(1)) {
            "REUSE" -> {
                ...
                if (sink != expectedSink) null else {
                    logger?.i("[+] Reusing PulseAudio listener module $id on $ip:$port")
                    ListenerCandidate(port, sink, id, createdNow = false)
                }
            }
            "CREATED" -> {
                val id = parts.getOrNull(2)?.toIntOrNull() ?: return null
                ...
                if (sink != expectedSink) null else {
                    ...
                    ListenerCandidate(port, sink, id, createdNow = true)
                }
            }
            else -> {
    ...
    private suspend fun unloadListenerThroughTermux(owner: TermuxOwner, moduleId: Int, logger: ContainerLogger?) {
        val body = """
            ...
            echo "OK|UNLOAD|${'$'}ID"
        """.trimIndent()
        runTermuxCommand(owner, "unload-$moduleId", body, 2000, logger)
    }
    ...
    private suspend fun runTermuxCommand(
        ...
        repeat(loops) {
            ...
            if (r?.isSuccess == true && r.out.isNotEmpty()) {
                ...
                val result = lines.lastOrNull { it.startsWith("OK|") || it.startsWith("ERR|") }
                    ?: lines.last()
    ...
    private fun verifyContainerClientDetailed(
        containerName: String,
        server: String,
        expectedSink: String,
        logger: ContainerLogger?

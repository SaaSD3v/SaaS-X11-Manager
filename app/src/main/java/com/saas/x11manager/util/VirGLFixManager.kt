    ...
    private suspend fun startHost(
        ...
        val command = """
            ...
            (
                ...
                exec ${q(VIRGL_BIN)}$optionalSuffix --socket-path ${q(HOST_SOCKET)}
    ...
    private suspend fun ensureHostRuntime(
        ...
        ownedHostReady(runtime)?.let { lease ->
            ...
            if (blockers.isNotEmpty()) {
                logger?.w("[VIRGL] ! Renderer flag change is pending; renderer is still in use by ${blockers.joinToString(", ")}")

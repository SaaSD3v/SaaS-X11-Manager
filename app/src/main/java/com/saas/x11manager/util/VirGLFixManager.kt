...
private fun socketInode(path: String): String? = try {
    ...
    private fun q(value: String): String =
        ...
        val probe = """
            ...
            found=''
            count=0
            for pid in $dollar(pidof virgl_test_server_android 2>/dev/null || true); do
                case "${dollar}pid" in ''|*[!0-9]*) continue ;; esac
                [ -r "/proc/${dollar}pid/status" ] && [ -r "/proc/${dollar}pid/cmdline" ] || continue
                uid=$dollar(sed -n 's/^Uid:[[:space:]]*\([0-9][0-9]*\).*/\1/p' "/proc/${dollar}pid/status" | sed -n '1p')
                [ "${dollar}uid" = 0 ] || continue
                cmd=$dollar(tr '\000' ' ' < "/proc/${dollar}pid/cmdline" 2>/dev/null || true)
                case " ${dollar}cmd " in
                    *"${dollar}target"*--socket-path*"${dollar}socket"*) ;;
                    *) continue ;;
                esac
                owns=0
                for fd in /proc/${dollar}pid/fd/*; do
                    [ -L "${dollar}fd" ] || continue
                    link=$dollar(readlink "${dollar}fd" 2>/dev/null || true)
                    [ "${dollar}link" = "socket:[${dollar}inode]" ] && { owns=1; break; }
                done
                [ "${dollar}owns" = 1 ] || continue
                start=$dollar(awk '{print ${dollar}22}' "/proc/${dollar}pid/stat" 2>/dev/null || true)
                [ -n "${dollar}start" ] || continue
                count=$dollar((count + 1))
                found="${dollar}pid:${dollar}start"
            done
            [ "${dollar}count" = 1 ] || exit 1
            printf '%s\n' "${dollar}found"
        """.trimIndent()

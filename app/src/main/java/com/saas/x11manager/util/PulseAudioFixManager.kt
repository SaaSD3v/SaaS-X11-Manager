...
object PulseAudioFixManager {
    ...
    suspend fun prepareBeforeGraphicalStart(
        ...
        logger?.i("--- PulseAudio Fix ---")\n        val totalStarted = System.nanoTime()\n        logger?.i("[AUDIO] • Engine: conventional DroidSpaces /tmp/.pulse-socket bridge")
        ...
        logger?.i("[AUDIO] • Step 1/4: checking Termux PulseAudio tools")\n        val prepareStarted = System.nanoTime()
        ...
        if (info.isRunning && original != "ON") {
            ...
            logger?.i("[AUDIO] ✓ Conventional PulseAudio configuration prepared")\n            logger?.i("[AUDIO] • Pre-start preparation time: ${(System.nanoTime() - totalStarted) / 1_000_000L}ms")
        }
    ...
    suspend fun finalizeAfterContainerReady(
        ...
        logger?.i("[AUDIO] ✓ Host /tmp/.pulse-socket reachable in ${(System.nanoTime() - hostWaitStarted) / 1_000_000L}ms")\n        val sink = selectAndroidSink(runtime)
            ?: return@withContext failure(logger, "No Android PulseAudio sink became available")

        logger?.i("[AUDIO] • Android sink: $sink")\n        val guestWaitStarted = System.nanoTime()\n        if (!waitForContainerBridge(containerName)) {\n            logger?.w("[AUDIO] • Guest /tmp/.pulse-socket verification timed out after ${(System.nanoTime() - guestWaitStarted) / 1_000_000L}ms")
            return@withContext failure(logger, "DroidSpaces did not expose /tmp/.pulse-socket inside $containerName")
        }

        logger?.i("[AUDIO] ✓ Guest /tmp/.pulse-socket visible in ${(System.nanoTime() - guestWaitStarted) / 1_000_000L}ms")\n        if (!installAndVerifyContainerClients(containerName, logger)) {
            return@withContext failure(logger, "Container PulseAudio client setup failed")
        }

        logger?.i("[AUDIO] ✓ PulseAudio ready ($sink)")\n        logger?.i("[AUDIO] • Final verification time: ${(System.nanoTime() - totalStarted) / 1_000_000L}ms")

...
object PulseAudioFixManager {
    ...
    suspend fun reconcileForStart(
        ...
        logger?.i("[AUDIO] • Engine: validated bundled helper v$SCRIPT_VERSION (pre-c679 baseline)")
        logger?.i("[AUDIO] • Helper SHA-256: $SCRIPT_SHA256")
        logger?.i("[AUDIO] • Preparing helper payload and Manager root broker")
        val stageStarted = System.nanoTime()
        val staged = stageHelper(context)
        logger?.i("[AUDIO] • Helper staging time: ${(System.nanoTime() - stageStarted) / 1_000_000L}ms")
        if (!staged.success) {
        ...
        logger?.i("[AUDIO] • Running SaaS DroidSpaces Audio Auto in mode=auto")
        val helperStarted = System.nanoTime()
        ...
        logger?.i("[AUDIO] • Helper execution time: ${(System.nanoTime() - helperStarted) / 1_000_000L}ms")
        if (!applied.success) {
        ...
        logger?.i("[AUDIO] ✓ Validated helper completed and verified the audio bridge")
        PulseAudioFixResult(
            success = true,
